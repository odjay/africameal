...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	try {
		...
		$template = "Bonjour merci de m'inscrire sur le fil d'actualité.<br><br>{formdata}<br><br>Date: {date}<br>Remote IP: {ip}<br><br>Salutations";
		...
		$from = ("1" == "1" ? $_POST["email"] : "odjay@hotmail.com");
		...
		// Send the mail
		if ( mail("info@africameal.ch", $subject, $body, $headers) ) {
			json(true);
		}
	}
	catch (Exception $e) {
